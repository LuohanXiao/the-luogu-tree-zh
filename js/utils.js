...
function respecBuyables(layer) {
	...
	if (!player[layer].noRespecConfirm && !confirm(tmp[layer].buyables.respecMessage || "你确定重置吗？这也会强制你进行一次\"" + (tmp[layer].name ? tmp[layer].name : layer) + "\"重置！")) return
...
function updateMilestones(layer) {
	...
	for (id in layers[layer].milestones) {
		if (!(hasMilestone(layer, id)) && layers[layer].milestones[id].done()) {
			...
			if (tmp[layer].milestonePopups || tmp[layer].milestonePopups === undefined) doPopup("milestone", tmp[layer].milestones[id].requirementDescription, "里程碑取得！", 3, tmp[layer].color);
...
function updateAchievements(layer) {
	...
	for (id in layers[layer].achievements) {
		if (isPlainObject(layers[layer].achievements[id]) && !(hasAchievement(layer, id)) && layers[layer].achievements[id].done()) {
			...
			if (tmp[layer].achievementPopups || tmp[layer].achievementPopups === undefined) doPopup("achievement", tmp[layer].achievements[id].name, "成就取得！", 3, tmp[layer].color);
		}
	}
}
