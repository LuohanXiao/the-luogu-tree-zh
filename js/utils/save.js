	...
	for (item in data) {
		if (data[item] == null) {
			...
			if (!NaNalert) {
				...
				NaNalert = true;
				alert("存档中发现了不合法内容，名称为： '" + item + "'。 请通知 mod 作者！你可以刷新页面，然后就不会出错了。")
				return
			}
		}
...
function importSave(imported = undefined, forced = false) {
	...
	try {
		...
		if (tempPlr.versionType != modInfo.id && !forced && !confirm("这个存档看起来来源于另一个 mod！你确定要导入吗？")) // Wrong save (use "Forced" to force it to accept.)
			return;
		player = tempPlr;
