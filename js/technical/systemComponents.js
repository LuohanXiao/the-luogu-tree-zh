var systemComponents = {
	...
	'layer-tab': {
		...
		<div v-if="back"><button v-bind:class="back == 'big' ? 'other-back' : 'back'" v-on:click="goBack(layer)">返回</button></div>
	...
	'overlay-head': {
		...
		<span v-if="player.devSpeed && player.devSpeed != 1" class="overlayThing">
			<br>开发者变速: {{format(player.devSpeed)}}x<br>
		</span>
    ...
    'options-tab': {
        ...
        <table>
            ...
            <tr>
                <td><button class="opt" onclick="switchTheme()">主题: {{ getThemeName()=='default'?'默认':'海蓝' }}</button></td>
                <td><button class="opt" onclick="adjustMSDisp()">展示里程碑: {{
                	MS_DISPLAYS[MS_SETTINGS.indexOf(options.msDisplay)]=='ALL'?'全部':(
                	MS_DISPLAYS[MS_SETTINGS.indexOf(options.msDisplay)]=='LAST, AUTO, INCOMPLETE'?'上一个，自动化，未完成':(
                	MS_DISPLAYS[MS_SETTINGS.indexOf(options.msDisplay)]=='AUTOMATION, INCOMPLETE'?'自动化，未完成':(
                	MS_DISPLAYS[MS_SETTINGS.indexOf(options.msDisplay)]=='INCOMPLETE'?'未完成':('无'))))

                }}</button></td>
                <td><button class="opt" onclick="toggleOpt('hqTree')">高质量树: {{ options.hqTree?"开":"关" }}</button></td>
            </tr>
            <tr>
                <td><button class="opt" onclick="toggleOpt('hideChallenges')">完成的挑战: {{ options.hideChallenges?"隐藏":"展示" }}</button></td>
                <td><button class="opt" onclick="toggleOpt('forceOneTab'); needsCanvasUpdate = true">单页模式: {{ options.forceOneTab?"总是":"自动" }}</button></td>
				<td><button class="opt" onclick="toggleOpt('forceTooltips'); needsCanvasUpdate = true">Shift+单击开关层级提示: {{ options.forceTooltips?"开":"关" }}</button></td>
				</tr> 
        </table>`
    },
    ...
    'back-button': {
        template: `
        <button v-bind:class="back" onclick="goBack()">返回</button>
        `
    },
